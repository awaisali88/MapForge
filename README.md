...
- **MGRS grid** — a two-tier real UTM/GZD tessellation: a static GZD graticule (zone/band boundaries with Norway and Svalbard zone exceptions) plus a dynamic fine grid via the custom `mgrstile://` vector-tile protocol (powered by `mgrs` + `pbf`). The resolution steps through a 7-level ladder (100 km → 50 km → 10 km → 1 km → 500 m → 200 m → 100 m) automatically as you zoom, or can be fixed manually via a sub-control. A small bottom-right indicator (just above the cursor readout) shows the active resolution whenever the grid is on, and the cursor readout itself switches from decimal degrees to MGRS.
