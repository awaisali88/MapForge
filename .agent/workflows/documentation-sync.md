...
| Runtime error names a library (e.g. `maplibre-gl:...`, `<Lucide Icon>`)                           | Pull that library's current docs via Context7 before guessing at the fix. Library packaging breaks (CJS↔ESM, peer-dep changes, factory-method rewrites) are the usual culprits. |
