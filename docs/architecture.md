...
The grid line spacing is set in metres via `setMgrsCellMeters` (decoupled from the MGRS label digit precision, which `cellMetersToDigits` derives so non-decade cell sizes like 200 m / 500 m / 50 km work). `useMgrsGrid` drives this through a 12-step ladder (1000 km → 500 km → 200 km → 100 km → 50 km → 10 km → 5 km → 2 km → 1 km → 500 m → 200 m → 100 m); the ≥ 200 km steps are sparse because UTM zones are only ~668 km wide, so the GZD zone boundaries carry the vertical structure there.
...
**MGRS tunables** (persisted): `mgrsAuto` (derive the resolution level from zoom vs. fixed), `mgrsLevel` (0–11 index into the 12-step ladder, 0=1000 km … 11=100 m), and the appearance settings `mgrsLineColor` / `mgrsLineWidth` / `mgrsLabelSize` (exposed in the settings drawer and pushed live onto the line + label layers via `setPaintProperty` / `setLayoutProperty`). While the grid is on, `useMgrsGrid` exposes a reactive `resolutionLabel` (e.g. `"1 km"`) that `MapView.vue` shows in the bottom-right `MgrsResolutionIndicator`, just above the cursor readout.
